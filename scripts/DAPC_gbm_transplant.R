...
personalized_ghosts(gbm.dapc, plot.mns=plot.mns) #for gbm
personalized_ghosts(ge.dapc, plot.mns=plot.mns)  #for ge 
...
#match scores between clone pairs are (somewhat) positively correlated
#this is not surprising given the strong genotypic effect of GBM
#But since Match-score correlates with fitness in transplants,
#How close your clone-mate is to the 'target' methylome could
#predict how well a transplant matches, and therefore its fitness
#This leaves question, what's more important, already matching,
#or shifting more to match
...
save(x, file='~/Desktop/shift.Rdata')

########## CORRELATIONS BETWEEN DAPC AXES FROM DIFFERENT DATASETS ##########
#BETWEEN GBM AND GE
head(traits)
par(mfrow=c(1,1))
#general LD coordinates
plot_ld_fitness(traits, 'gbm.ld1', ld.col= 'ge.ld1', legend.pos='topleft')
#match scores
mclim = merge(clim.ge, clim.gbm, by = 'Colony.ID')
plot(mclim$z.y~mclim$z.x, pch=21, bg=get.cols(mclim$treat.x), cex=2, xlab='GE Match-score', ylab='GBM Match-score')
lmm=lm(mclim$z.y~mclim$z.x);summary(lmm);abline(lmm, lty=2, lwd=2)
#shifts
mshift=merge(shift.ge, shift.gbm, by = 'geno')
plot(mshift$shift.y~ mshift$shift.x, pch=21, bg=get.cols(mclim$treat.x), cex=2, xlab='GE Shift', ylab='GBM Shift')
lms=lm(mshift$shift.y~ mshift$shift.x);summary(lms);abline(lms, lty=2, lwd=2)

#BETWEEN GBM AND SNPS
#general LD coordinates
plot_ld_fitness(traits, 'gbm.ld1', ld.col= 'snp.ld1', legend.pos='topleft')
#match scores
mclim = merge(clim.snp, clim.gbm, by = 'Colony.ID')
plot(mclim$z.y~mclim$z.x, pch=21, bg=get.cols(mclim$treat.x), cex=2, xlab='SNP Match-score', ylab='GBM Match-score')
sub=mclim[mclim$ori.x=='K',]
lmm=lm(sub$z.y~sub$z.x);summary(lmm);abline(lmm, lty=2, lwd=2, col=color.set[2])
sub=mclim[mclim$ori.y=='O',]
lmm=lm(sub$z.y~sub$z.x);summary(lmm);abline(lmm, lty=2, lwd=2, col=color.set[3])

#no clear conclusion from this.

################ VARIABLE CONTRIBUTIONS ################
#DAPC also calculates the contribution of each variable
#(ie the disciminatory power of that variable)
#which genes are highly disciminating for KK vs OO?
contr=data.frame(dp$var.contr[order(dp$var.contr, decreasing=T),])
colnames(contr) = c('con')
head(contr)
c= merge_protein_names(contr, sort.col='con', DECREASING=T)
c[1:25,]
...
l = 'LOC107338192'
fit.proxy='GAIN'
sub=data.frame(vsd[l,])
colnames(sub) = c(l)
rownames(sub) = sub('_2m', '', rownames(sub))
sub$Colony.ID=rownames(sub)
colors=substr(rownames(sub), start=1, stop=2)
...
for (l in rownames(c)[1:5]){
	sub=data.frame(vsd[l,])
	colnames(sub) = c(l)
	rownames(sub) = sub('_2m', '', rownames(sub))
	sub$Colony.ID=rownames(sub)
	x=merge(traits, sub, by='Colony.ID')
	par(mfrow=c(3,2))
	plot_ld_fitness(x, 'GAIN', ld.col=l)
	title(main=l)
	plot_ld_fitness(x, 'CARB', ld.col=l)
	plot_ld_fitness(x, 'PROTEIN', ld.col=l)
	plot_ld_fitness(x, 'LIPID', ld.col=l)
	plot_ld_fitness(x, 'ZOOX', ld.col=l)
}
...
#LOOK AT MORTLAITY
boxplot(clim.gbm$z~ clim.gbm$mort, ylab='Acclimatization Z-score', mgp=MGP)
ko= clim.gbm[clim.gbm$treat=="KO",]
ok= clim.gbm[clim.gbm$treat=="OK",]
transplants=rbind(ko,ok)
boxplot(transplants$z~ transplants$mort, ylab='Acclimatization Z-score', mgp=MGP)
boxplot(ko$z~ko$mort, ylab='Acclimatization Z-score', mgp=MGP)
boxplot(ok$z~ok$mort, ylab='Acclimatization Z-score', mgp=MGP)



#LOOK AT CORRELATION BETWEEN DAPC RESULTS FOR GE AND MBD
colors=traits$treat
colors[colors=='KK']<-color.set[1]
colors[colors=='KO']<-color.set[2]
colors[colors=='OK']<-color.set[3]
...
#plot overall correlation (this is expected since they were desinged to find same variation)
plot(traits$gbm.ld1~traits$ge.ld1, pch=21, bg=colors, col='black')
lm1=lm(traits$gbm.ld1~traits$ge.ld1)
summary(lm1)
abline(lm1)
