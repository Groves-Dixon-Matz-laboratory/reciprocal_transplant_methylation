...
directory<-"/Users/grovesdixon/lab_files/projects/recip_meth/deseq_12_13_16/correlation_with_expression"
setwd(directory)

#READ IN THE COUNTS DATA
counts=read.table('all_rnaseq_counts.txt',header=TRUE,row.names=1) 
head(counts)
dim(counts)


######### subset data to match those with methylation results ##########
# mdat = read.table("CDS_counts_11-14-16.tsv",header=T,row.names=1)
# mdat = colnames(mdat)
# mdat
# inmet = c()
# for (m in mdat){
	# name = strsplit(m, "_")[[1]][1]
	# inmet = append(inmet, name)
# }
# length(inmet)
# counts = counts[colnames(counts) %in% inmet]
# head(counts)
# dim(counts)
##################################################
...
save(orico, file="orico_RNAseqV2.Rdata")
save(traco, file="traco_RNAseqV2.Rdata")
...
head(orico)
head(traco)
save(orico, file="orico_RNAseq.Rdata")
save(traco, file="traco_RNAseq.Rdata")


rna.rld=rlog(ddsco)
head(rna.rld)
rna.rld=assay(rna.rld)
colnames(rna.rld) = colnames(counts)
head(rna.rld)
dim(rna.rld)
save(rna.rld, COLDATA, ddsco, counts, file="RNAseq_objects.Rdata")
...
library(ggplot2)
NTOP = 25000
plotPCA(rl,intgroup=c("origin","transplant"),ntop=25000)+theme_bw()
