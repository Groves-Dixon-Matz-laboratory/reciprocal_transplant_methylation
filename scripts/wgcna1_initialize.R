...
lnames=load("datasets/gbm_counts_p-1000_200.Rdata") #output from DEseq_unbound_v_flowthrough.R
lnames
counts=gcounts
...
coldata=coldata[,10:ncol(coldata)]
...
save.image("datasets/wgcnaInitialize_p1000_200_iterate_mnCount10.Rdata")

#---------------------- IDENTIFY OUTLIERS AS IN WGCNA TUTORIAL -----------------------
directory<-"~/gitreps/reciprocal_transplant_methylation/"
setwd(directory)
# lnames=load("wgcnaInitialize_p1000_200_iterate_mnCount6.Rdata")
lnames=load("datasets/wgcnaInitialize_p1000_200_iterate_mnCount10.Rdata")
...
ori=as.character(coldata$origin)
ori[ori=="K"]<-15
ori[ori=="O"]<-19
ori=as.numeric(ori)
trans = as.character(coldata$transK)
trans[trans=="0"]<-ocol
trans[trans=="1"]<-kcol
m.pca = mod.plotPCA.df(rld.df, coldata, intgroup="transK", returnData=T,ntop= NTOP)
...
box()
o=m.pca[,c(1,2)]
ordispider(ord=o, groups=coldata$colony.id, label = F, lwd=0.5, col='black')
...
save(datExpr, datTraits, file = "wgcna-01_output_p1000_200_iterate_mnCount10.RData") #use the Rdata file for input to run WGCNA on full dataset
